...
function make_index_dynamic_part(){
  $.post("get_user_info_from_session.php", {},  make_index_banner);
}

function make_person_center_dynamic_part(){
  $.post("get_user_info_from_session.php", {},  make_person_center_banner);
}
...
function make_index_banner(ret_str){
  ...
  if(items.length != 2){
    // 用户没有登录
      content= '<div style="margin-top:34px; margin-left:150px; float:right; ">' +
              '<p style="font: normal normal normal 18px/23px, "SimSun", SimSun;padding-top: 35px; padding-bottom: 35px; " >' +
              '<a class="theme-login" href="javascript:;" ><img src="images/icon/dl.png" style="height:20px; vertical-align:middle; width:20px;"/><span style="margin-left:5px; vertical-align:middle;color:black;">登 陆</span></a> |  ' +
              '<a class="theme-login" id = "theme-login_1" ref="javascript:;" ><img src="images/icon/zc.png" style="vertical-align:middle;height:20px; width:20px;" /><span style="margin-left:5px; vertical-align:middle;color:black;">注 册</span></a></p>' +
              '</div>' +
              ...
              '    <form class="theme-signin t1" style="margin-left:90px; margin-top:-30px;" name="loginform" action="login.php" method="post">' +
              '          <ol>' +
              ...
              '     <form class="theme-signin t2"  style="margin-left:10px; margin-top:-50px;" name="loginform" onsubmit="return reg_param_check()" action="register.php" method="post">' +
              '              <ol>' +  
    ...
    if(items[1] == "s"){
      ...
      content = '<div style="margin-top:34px; margin-left:60px; float:right;"><p style="font: normal normal normal 18px/23px "SimSun", SimSun;padding-top: 35px; padding-bottom: 35px;">您好,&nbsp'+ items[0] +' | <a href="judge_stu_or_teacher.php"><span style="color:red">棋院管理</span></a>|<a href="session_destory.php"><span style="color:black;">注销</span></a></p></div>'; 
    }else{
      // 错误
    }
  }
...
function make_person_center_banner(ret_str){
  ...
  if(items.length != 2){
    ...
    content='<div style="margin-right:30px; float:right; border:0px solid;"><p class="p_sty" style="padding-top:35px; padding-bottom: 35px;">您好,&nbsp'+ items[0] +' | <a href="judge_stu_or_teacher.php"><span style="color:red">点击上课</span></a>|<a href="index.html"><span style="color:black;">返回主页</span></a></p></div>';
  }
  // 添加到html
  $('#title').after(content);
}